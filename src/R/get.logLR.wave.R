...
#st.sites=1
#en.sites=100
#path.output.dir='/mnt/lustre/home/shim/multiscale_analysis/analysis/roger_ATAC/run/wave/Copper.2048.plus.alt.run/'
...
for(i in st.sites:en.sites){
    ...
    if(file.exists(path.each)== FALSE){		
        ...
        if(file.info(path.each)$size == 0){
            ...
            dat = read.table(path.each, as.is = TRUE)
            if(!is.na(dat[1,2])){
                logLR[i] = as.numeric(dat[1,2])
                done[i] = TRUE
            }else{
                done[i] = FALSE
            }
