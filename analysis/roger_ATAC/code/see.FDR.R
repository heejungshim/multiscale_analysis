...
## Aim : see FDR curves of WaveQTL, multiseq, DESeq2 with different window sizes
##
...
del.ix = union(del.ix, which(is.na(pval.deseq.100.10)==TRUE))
del.ix = union(del.ix, which(is.na(pval.deseq.3.0)==TRUE))
del.ix = union(del.ix, which(is.na(pval.deseq.3.10)==TRUE))
...
qval.deseq.3.10 = qvalue(pval.deseq.3.10[-del.ix])
qval.deseq.100.0 = qvalue(pval.deseq.100.0[-del.ix])
qval.deseq.100.10 = qvalue(pval.deseq.100.10[-del.ix])
...
length(alpha.list)
# 20
num.wave = num.ms = num.deseq.full.0 = num.deseq.3.0 = num.deseq.3.10 = num.deseq.100.0 = num.deseq.100.10 = rep(NA, length(alpha.list))
for(i in 1:length(alpha.list)){
    ...
    num.deseq.3.10[i] = sum(qval.deseq.3.10$qvalues < alpha.list[i])
    num.deseq.100.0[i] = sum(qval.deseq.100.0$qvalues < alpha.list[i])
    num.deseq.100.10[i] = sum(qval.deseq.100.10$qvalues < alpha.list[i])
}
...
ymax = max(num.wave, num.ms, num.deseq.full.0, num.deseq.3.0, num.deseq.3.10, num.deseq.100.0, num.deseq.100.10)
ymin = 0
...
points(alpha.list, num.deseq.3.10, ylim=c(ymin,ymax), col="orange", type="l", lty=3)
points(alpha.list, num.deseq.100.0, ylim=c(ymin,ymax), col="purple", type="l")
points(alpha.list, num.deseq.100.10, ylim=c(ymin,ymax), col="purple", type="l", lty=3)
legend(0.01, ymax*0.99, c("multiseq", "WaveQTL", "DESeq full 0", "DESeq 300 0", "DESeq 300 10", "DESeq 100 0", "DESeq 100 10") , col = c("red", "blue", "darkgreen", "orange", "orange", "purple", "purple"), lty = c(1,1,1, 3, 1, 3), text.col = "black",merge = FALSE, bg = "white")
...
legend(0.01, ymax*0.99, c("multiseq", "WaveQTL", "DESeq full 0", "DESeq 600 0", "DESeq 300 0", "DESeq 100 0") , col = c("red", "blue", "darkgreen", "brown", "orange", "purple"), lty = c(1,1,1, 1, 1, 1), text.col = "black",merge = FALSE, bg = "white")
