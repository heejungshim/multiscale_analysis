...
## Aim : run DESeq2 with 600bp windows. I'll focus on `Copper.2048.both'. 
##
...
numC.n = 3
deseq.data.combine = matrix(data=NA, nc = numSam, nr = sum(numSites.list)*numC.n)
for(ss in 1:numSam){
  tmp.dat = matrix(deseq.data[,ss], ncol= numC, byrow =T)
  tmp2.dat = tmp.dat
  tmp2.dat[,18] = tmp2.dat[,18] + tmp2.dat[,19] + tmp2.dat[,20]
  tmp3.dat = matrix(as.vector(t(tmp2.dat[,1:18])), ncol = 6, byrow = T)
  deseq.data.combine[,ss] = apply(tmp3.dat,1,sum)
}
...
pval.filtered =matrix(pval.vec,ncol=numC.n,byrow=T)
...
## output min.pval
write.table(min.pval, file = paste0(output.dir.path, "/DESeq2.600.min.pval.", filter.cut, ".txt"), quote= FALSE, row.names = FALSE, col.names = FALSE)

## output pval
write.table(pval.filtered, file = paste0(output.dir.path, "/DESeq2.600.all.pval.", filter.cut, ".txt"), quote= FALSE, row.names = FALSE, col.names = FALSE)
