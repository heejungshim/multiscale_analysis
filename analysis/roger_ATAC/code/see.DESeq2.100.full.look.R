...
out.path = paste0("/mnt/lustre/home/shim/multiscale_analysis/analysis/roger_ATAC/run/deseq/", treatment, ".", siteSize, ".", strand, ".", 100, ".alt.run/output/DESeq2.pval.discrete.Robj")
...
pdf("hist.statistic.pval.DESeq2.100.full.discrete.pdf")

#### filter.cut = 0 window 100

filter.cut = 0
pval = pval.deseq.100.0
...
#### filter.cut = 0 full window

filter.cut = 0
pval = pval.deseq.full.0

deseq.alt = as.numeric(read.table(paste0("/mnt/lustre/home/shim/multiscale_analysis/analysis/roger_ATAC/run/deseq/", treatment, ".", siteSize, ".", strand, ".", 100, ".alt.run/output/DESeq2.full.pval.", filter.cut, ".txt"))[,1])
deseq.null = as.numeric(read.table(paste0("/mnt/lustre/home/shim/multiscale_analysis/analysis/roger_ATAC/run/deseq/", treatment, ".", siteSize, ".", strand, ".", 100, ".null.run/output/DESeq2.full.pval.", filter.cut, ".txt"))[,1])
...
par(mfrow = c(3,1))
hist(deseq.alt[-del.ix.deseq], breaks = 200, main=paste0(filter.cut, " : alt test statistic [2048]"), xlim=c(xmin, xmax), xlab ="p-value")
hist(deseq.null[-del.ix.deseq], breaks = 200, main=paste0(filter.cut, " : null test statistic [2048]"), xlim=c(xmin, xmax), xlab ="p-value")
hist(pval[-del.ix.deseq], breaks = 200, main=paste0(filter.cut, " : pvalue [2048]", length(pval)-length(del.ix.deseq)), xlim=c(xmin, xmax), xlab ="p=value")
...
setwd("/mnt/lustre/home/shim/multiscale_analysis/analysis/roger_ATAC/fig/DESeqdebug/")
pdf("hist.statistic.pval.DESeq2.100.full.discrete.pooling.pdf")
